...
def _check(cfg: Config) -> int:
    ...
    if cfg.tts_backend == "elevenlabs":
        if cfg.elevenlabs_api_key and speech._afplay_available():
            print(f"✅ ElevenLabs speech configured: {cfg.elevenlabs_model}")
        else:
            print("❌ ElevenLabs speech needs ELEVENLABS_API_KEY and macOS `afplay`")
            ok = False
    elif speech.say_available():
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        "--tui", "--terminal", action="store_true", dest="tui",
        help="Launch the sleek interactive terminal UI (type or speak commands).",
    )
    parser.add_argument(
        "--web", action="store_true",
        help="Launch the Atlas web UI in your browser instead of the voice loop.",
    )
    parser.add_argument(
