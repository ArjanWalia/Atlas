...
Prefer to stay in the terminal? Launch the sleek, voice-first interface — a
Claude-Code-style UI with a blue cat, an animated spinner, and rich output:

```bash
python run.py --tui
```

**Just start talking — no setup.** The moment it opens, Atlas brings up the
microphone and reads every reply aloud, so a blind user can use it hands-free
from the first second. Say **"exit atlas"** (or press Ctrl+C) to quit.

If no microphone is available, it falls back to a typed prompt with a few slash
commands:

| Command | What it does |
|---------|--------------|
| `/say TEXT` | Speak a line in the Atlas voice |
| `/clear` | Clear the screen |
| `/help` | Show all commands |
| `/exit` | Quit (or say "exit atlas") |

It reads each reply aloud (ElevenLabs or macOS `say`) and adds no dependencies —
it's pure standard-library ANSI.

### Web UI

Prefer a screen? Launch the premium browser interface — type or speak a request,
watch each stage (refined prompt → Cursor output → spoken summary), and hear the
reply played back in your ElevenLabs voice:

```bash
python run.py --web            # opens http://127.0.0.1:8000 in your browser
python run.py --web --port 9000
```

The web UI is built on the Python standard library (no extra dependencies), runs
entirely on your machine, and reuses the same pipeline as the voice loop. Voice
input in the browser uses the Web Speech API (Chrome/Edge); voice output uses
ElevenLabs when `ATLAS_TTS_BACKEND=elevenlabs`.
...
| `ATLAS_TTS_BACKEND` | `say` | `say` for macOS speech or `elevenlabs` for ElevenLabs TTS. |
| `ELEVENLABS_API_KEY` | — | Required when `ATLAS_TTS_BACKEND=elevenlabs`. |
| `ELEVENLABS_MODEL` | `eleven_flash_v2_5` | Efficient low-latency ElevenLabs TTS model. |
| `ELEVENLABS_VOICE_ID` | Rachel | ElevenLabs voice ID to use for speech. |
